...
function CategoryCard({ name, image, onClick, isActive = false }) {
  return (
    <div
      onClick={onClick}
      className={`group relative aspect-[4/5] w-[40vw] min-w-[120px] max-w-[180px] shrink-0 snap-start cursor-pointer overflow-hidden rounded-[1.4rem] transition-all duration-200 sm:w-[24vw] sm:max-w-[170px] md:w-[18vw] md:max-w-[180px] lg:w-[160px] surface-card-sm card-hover ${
        isActive
          ? 'ring-2 ring-emerald-200 ring-offset-2 border-emerald-400'
          : 'border-transparent'
      }`}
    >
      {image ? (
